...
namespace Ally::ServiceControl
{
    namespace c = AnyFSE::App::Constants;

    namespace detail
    {
        inline constexpr DWORD ServiceTimeoutMs = 30000;
        ...
        inline bool Fail(DWORD error)
        {
            SetLastError(error);
            return false;
        }

        inline bool QueryStatus(SC_HANDLE service, SERVICE_STATUS_PROCESS& status)
        {
            DWORD bytesNeeded = 0;
            return QueryServiceStatusEx(service, SC_STATUS_PROCESS_INFO, reinterpret_cast<BYTE*>(&status), sizeof(status), &bytesNeeded) != FALSE;
        }

        inline bool WaitForState(SC_HANDLE service, DWORD desiredState)
        {
        ...
        inline bool Start(SC_HANDLE service)
        {
            SERVICE_STATUS_PROCESS status = {};
            if (!QueryStatus(service, status)) return false;
        ...
        inline bool Stop(SC_HANDLE service)
        {
            SERVICE_STATUS_PROCESS status = {};
            if (!QueryStatus(service, status)) return false;
    ...
    inline bool CreateInjector(const std::wstring& injectorExe)
    {
        if (injectorExe.empty()) return detail::Fail(ERROR_INVALID_PARAMETER);
        detail::ServiceHandle manager(OpenSCManagerW(nullptr, nullptr, SC_MANAGER_CONNECT | SC_MANAGER_CREATE_SERVICE));
        ...
        detail::ServiceHandle service(OpenServiceW(manager.value, c::InjectorServiceName, access));
        if (!service.value)
        {
        ...
        return detail::Start(service.value);
    }

    inline bool RemoveInjector()
    {
        detail::ServiceHandle manager(OpenSCManagerW(nullptr, nullptr, SC_MANAGER_CONNECT));
        if (!manager.value) return false;
        detail::ServiceHandle service(OpenServiceW(manager.value, c::InjectorServiceName, SERVICE_STOP | SERVICE_QUERY_STATUS | DELETE));
        if (!service.value) return GetLastError() == ERROR_SERVICE_DOES_NOT_EXIST;
        if (!detail::Stop(service.value)) return false;
        if (!DeleteService(service.value)) return GetLastError() == ERROR_SERVICE_MARKED_FOR_DELETE;
        return true;
    }

    inline bool EnableAsusOptimization()
    {
        detail::ServiceHandle manager(OpenSCManagerW(nullptr, nullptr, SC_MANAGER_CONNECT));
        if (!manager.value) return false;
        detail::ServiceHandle service(OpenServiceW(manager.value, c::AsusOptimizationService, SERVICE_CHANGE_CONFIG | SERVICE_START | SERVICE_QUERY_STATUS));
        if (!service.value) return false;
        if (!ChangeServiceConfigW(service.value, SERVICE_NO_CHANGE, SERVICE_AUTO_START, SERVICE_NO_CHANGE,
